...
public class MainActivity extends Activity {
  ...
  @Override public void onCreate(Bundle b){
    ...
    mainWebView.loadUrl("file:///android_asset/index.html"); setContentView(mainWebView);
  }
  ...
  private class AppWebViewClient extends WebViewClient {
    ...
    private boolean openExternal(Uri uri){
      ...
      if(scheme.equals("http")||scheme.equals("https")||scheme.equals("whatsapp")||scheme.equals("mailto")||scheme.equals("tel")){
        try{startActivity(new Intent(Intent.ACTION_VIEW,uri));}catch(ActivityNotFoundException e){Toast.makeText(MainActivity.this,"Hakuna app ya kufungua link hii.",Toast.LENGTH_SHORT).show();}
        return true;
      } return false;
    }
  }
  ...
  private class AppWebChromeClient extends WebChromeClient {
    @Override public boolean onShowFileChooser(WebView webView,ValueCallback<Uri[]> callback,FileChooserParams params){
      ...
      try{startActivityForResult(intent,FILE_CHOOSER_REQ);return true;}catch(ActivityNotFoundException e){filePathCallback=null;Toast.makeText(MainActivity.this,"File picker haipatikani kwenye kifaa hiki.",Toast.LENGTH_LONG).show();return false;}
    }
  }
  ...
  @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){
    ...
    if(requestCode==FILE_CHOOSER_REQ){
      if(filePathCallback==null)return; Uri[] results=null;
  ...
  private class AndroidBridge {
    @JavascriptInterface public void openFilePicker(){runOnUiThread(()->{
      Intent intent=new Intent(Intent.ACTION_OPEN_DOCUMENT);intent.addCategory(Intent.CATEGORY_OPENABLE);intent.setType("text/*");
      intent.putExtra(Intent.EXTRA_MIME_TYPES,new String[]{"text/csv","text/plain","application/csv","application/vnd.ms-excel","application/octet-stream"});
    ...
    @JavascriptInterface public void shareWhatsApp(String message,String phone){try{
      String p=phone==null?"":phone.replaceAll("[^0-9]",""); Uri uri=Uri.parse("https://wa.me/"+p+"?text="+Uri.encode(message==null?"":message));
    ...
    @JavascriptInterface public void savePdf(String html,String filename){runOnUiThread(()->{
      printWebView=new WebView(MainActivity.this);WebSettings s=printWebView.getSettings();s.setJavaScriptEnabled(true);s.setDomStorageEnabled(true);printWebView.setVisibility(View.INVISIBLE);
      printWebView.setWebViewClient(new WebViewClient(){@Override public void onPageFinished(WebView view,String url){
        PrintManager pm=(PrintManager)getSystemService(PRINT_SERVICE);String job=(filename==null||filename.isEmpty()?"Finesty Sales Report":filename).replace(".pdf","");
        PrintDocumentAdapter adapter=view.createPrintDocumentAdapter(job);pm.print(job,adapter,new PrintAttributes.Builder().setMediaSize(PrintAttributes.MediaSize.ISO_A4).setMinMargins(PrintAttributes.Margins.NO_MARGINS).build());
    ...
    @JavascriptInterface public void requestPermissions(){
      if(Build.VERSION.SDK_INT>=33){
        if(checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED) MainActivity.this.requestPermissions(new String[]{Manifest.permission.POST_NOTIFICATIONS},NOTIFICATION_REQ);
    ...
    @JavascriptInterface public String getPermissionStatus(){
      String n;if(Build.VERSION.SDK_INT>=33)n=checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)==PackageManager.PERMISSION_GRANTED?"Notifications: GRANTED":"Notifications: NOT GRANTED";else n="Notifications: SYSTEM MANAGED";
